...
public class IsAlgorithm {
    ...
    public IsAlgorithm(int n, int m, ArrayList<ArrayList<Integer>> adjacencyTable, String potential_community, String core_nodes) {
        this.node_size = n;
        this.link_size = m;
    ...
    public ArrayList<ArrayList<Integer>> run_is(int seed_id) {
        ...
        //20200124 add candidate seed node algorithm  20200624 change basic method to connected component
...
//        neighbors_conectivity_in = neighbors_in.size();
    ...
    public ClusterCore getClusterCore(int node, ArrayList<NodeInfluence> node_influences_id) {
        ...
        //20200714 experiments prove this is useless
        ...
        clusterCore = getConnectedComponents(nodes_list);
        ...
        //20200715 add potential community to seed selection procedure
        TreeSet<Integer> temp = new TreeSet<>();
        cCore.setList(initial_community(node, cCore.getList(), temp, node_influences_id));
        return cCore;
    }
...
//            temp_influences = calculateClusterCoreInfluence(tempClusterCore, node_influences_id);
    ...
    //社区向外的度作为社区影响力 20200630 略有下降 暂时不用（以社区内全部度为影响力更为科学）
    public int calculateClusterCoreInfluence(TreeSet<Integer> clusterCore, ArrayList<NodeInfluence> node_influences_id) {
        int influence = 0;
        for(int node : clusterCore) {
            influence += node_influences_id.get(node).influence;
            int internalDegree = get_insection(clusterCore, getNeighbors(node, adjacencyTable)).size();
            influence = influence - internalDegree;
        }
//        attraction = attraction/(double)getCommunityEdgeNodes(community).size();
        return influence;
    }

    //20200630 两节点的邻接点交集与所有邻接点的比值作为两节点相似度，并以节点相似度为基础，构建节点权重，社区权重及社区吸引力。
    public double calculateSimilarityBetweenNodes(int nodeA, int nodeB) {
        ...
        similarity = (double)get_insection(neighborsOfA, neighborsOfB).size() /
                ((double)neighborsOfA.size() + (double)neighborsOfB.size());
        return similarity;
    }

    public double getWeightOfNode(int node) {
        ...
        for(int neighborNode : neighbors) {
            weightOfNode += calculateSimilarityBetweenNodes(node, neighborNode);
        }
        return weightOfNode;
    }

    public double getWeightOfCommunity(TreeSet<Integer> community) {
        double weightOfCommunity = 0;
        for(int node : community) {
            weightOfCommunity += getWeightOfNode(node);
        }
        return weightOfCommunity;
    }

    public double getAttractionOfCommunityToNode(int node, TreeSet<Integer> community) {
        double attraction = 0;
        TreeSet<Integer> neighbors = new TreeSet<>();
        TreeSet<Integer> insections = new TreeSet<>();
        neighbors = getNeighbors(node, adjacencyTable);
        insections = get_insection(neighbors, community);
        for(int insectionsNode : insections) {
            attraction += getWeightOfNode(insectionsNode);
        }
        return attraction;
    }
    ...
    //20200701 DING(2020)
    public int getNodeCommunitySimilarity(int node, TreeSet<Integer> community) {
        int similarity = 0;
        ...
        nodeMass.add(node);
        nodeCommunityCommon = get_insection(nodeMass, community);
        for(int nodeId : nodeCommunityCommon) {
            ...
            commonArea = get_insection(nodeMass, neighborMass);
            similarity += commonArea.size();
        }
        return similarity;
    }
