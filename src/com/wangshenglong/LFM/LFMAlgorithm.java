...
public class LFMAlgorithm
{
	...
	public LFMAlgorithm(int n, int m,
	...
	{
		// System.out.println("seed" + "," + (node_id + 1));
		...
		while (max_fitness_node.fitness > 0)
		{
			...
			if (max_fitness_node.id != -1)
			{
				...
				reflash_graph_neighbors_add(nature_community,
						max_fitness_node.id);
				// System.out
				// .print("add" + "," + (max_fitness_node.id + 1) + ",[");
				// for (Iterator<Integer> iter = nature_community.community
			...
			while (min_fitness_node.fitness < 0)
			{
				...
				System.out.print("remove" + "," + (min_fitness_node.id + 1)
						+ ",[");
				for (Iterator<Integer> iter = nature_community.community
						.iterator(); iter.hasNext();)
				{
	...
	{
		String file_path = "E:/Algorithms results/Algorithms results (overlap)/real-world networks/";
		...
		double a = 1;
		LFMAlgorithm lfm = new LFMAlgorithm(data_reader.n, data_reader.m,
