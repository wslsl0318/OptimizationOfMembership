package com.wangshenglong.fifth;
...
public class FifthAlgorithm {
	...
	public FifthAlgorithm(int n, int m,
			...
			ArrayList<NodeImportance> nodes_importance) {
		this.node_size = n;
		this.link_size = m;
	...
	public double calculate_common_neighbors_density(TreeSet<Integer> insection) {
		...
		density = (double) 2 * number_of_links / (double) number_of_nodes * (number_of_nodes - 1);
//		density = (double) 2 * number_of_links / (double) (number_of_nodes * (number_of_nodes - 1));
		return density;
	}
	...
	public int find_replacement_seed_node_for_initial_seed_node_from_neighbors(
		...
		/***********************************************************************************/
		//20221006 Thinking of centrality first, and then similarity.(Old)
		for (Iterator<Integer> iter = neighbors_of_seed.iterator(); iter
				.hasNext();) {
			Integer neighbor = iter.next();
		...
		//20221006 Thinking of similarity first, and then centrality.(new)
		/*for (Iterator<Integer> iter = neighbors_of_seed.iterator(); iter
				.hasNext();) {
			Integer neighbor = iter.next();
			double relation_strength = calculate_relation_strength(node_id,
	...
	public double calculate_relation_strength_of_node_set(
		...
		/* do not use node influence area */
		/* nodes_with_higher_importance is changed into node_set */
		// ArrayList<Integer> nodes_with_higher_importance = new
		// ArrayList<Integer>();
		// for (Iterator<Integer> iter = node_set.iterator(); iter.hasNext();) {
		// Integer node = iter.next();
		// int importance_of_node = nodes_importance.get(node).importance; //
		// calculate_node_importance(node);
		// if (importance_of_node >= community_neighbor_importance) {
		// nodes_with_higher_importance.add(node);
		// }
		// }
